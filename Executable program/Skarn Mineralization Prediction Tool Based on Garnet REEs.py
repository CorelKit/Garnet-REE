...
if getattr(sys, 'frozen', False):
    dll_path = os.path.join(sys._MEIPASS, 'xgboost', 'lib', 'xgboost.dll')
...
class ExcelProcessorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("A Prediction Tool for Skarn Metallic Mineralization Based on Garnet Rare Earth Elements")
        self.root.geometry("800x491")
    ...
    def process_data(self):
        ...
        try:
            ...
            sel_columns_clean['La/Sm'] = sel_columns_clean['La'] / sel_columns_clean['Sm']
            sel_columns_clean['δEu'] = sel_columns_clean['Eu'] / np.sqrt(sel_columns_clean['Sm'] * sel_columns_clean['Gd'])
            sel_columns_clean['Y/Ho'] = sel_columns_clean['Y'] / sel_columns_clean['Ho']
            sel_columns_clean['Ho/Yb'] = sel_columns_clean['Ho'] / sel_columns_clean['Yb']
            ...
            blind_v, countnum = [], 0
            models = ['LightGBM_log10_smote_W', 'XGBoost_log10_smote_Sn', 'LightGBM_log10_smote_Mo',
                      'LightGBM_raw_smote_Fe', 'LightGBM_log10_weighting_Cu', 'XGBoost_raw_smote_Au',
                      'LightGBM_log10_smote_Pb', 'XGBoost_raw_smote_Zn']

            for num, model_cur in enumerate(models):
                countnum += 1
                pkl_path = current_dir + '/models/' + model_cur + '.bin'
                ...
                if os.path.exists(pkl_path):
                    ...
                    exit()
                if num in [0,1,2,4,6]:
                    y_zc_ree = np.log10(sel_columns_clean.iloc[:, 2:21])
                else:
                    y_zc_ree = sel_columns_clean.iloc[:, 2:21]
                y_val = loaded_model.predict(y_zc_ree)
                if countnum == 1:
            ...
            results_rf_df = pd.DataFrame(blind_v, columns=metal)
            results_rf_df.replace({0: "BW", 1: "S"}, inplace=True)
            count_stats = []
            for col in results_rf_df.columns:
                ...
                count_stats.append(f"{col}: BW={count_0}, S={count_1}")
            self.log_message(f"Predicted result matrix: "
                             f"\nBW: Barren to Weak Metallic Mineralization; "
                             f"\nS: Strong Metallic Mineralization."
                             f"\nMetal thresholds (T). "
                             f"\nWO3: 0.001 Mt, Sn: 0.001 Mt, Mo: 0.001 Mt, Fe: 0.01 Mt,"
                             f"\nCu: 0.001 Mt, Au: 1 t, Pb: 0.001 Mt, Zn: 0.001 Mt."
                             f"\nBW < T, S >= T."
                             f"\n{str(results_rf_df)}")
